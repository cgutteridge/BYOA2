 ...
 */
export class RandomItemPower extends ItemPower {
  // UI properties
  readonly displayName = "Item Generator";
  readonly icon = "🎁";
  ...
  useWithoutTarget(item: Item): boolean {
    ...
    questStore.updateStats(item.uses, 0, 0, `Used ${item.name} to generate ${newItem.name}.`);
